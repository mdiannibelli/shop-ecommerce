...
export default function CartPage() {
    ...
    useEffect(( ) => {
        ...
        const storedCartItems = JSON.parse(localStorage.getItem("cart")) || []; //cart is the key
    ...
    const handleDecrease = (item) => {
        if(item.quantity > 1) {
            ...
            //update local sorage with new items
            localStorage.setItem("cart", JSON.stringify(cartItems))
        }
    }
    ...
    //cart subtotal
    const cartSubtotal = cartItems.reduce((total, item) => { // operación acumulativa en los elementos del arreglo para producir un único resultado.
        return total + calculateTotalPrice(item) // *total* es el acumulador
    }, 0) // *initial value = 0*
                                    ...
                                    cartItems.map((item,i) => (
                                        <tr key={i}>
                                            ...
                                            <td className='cat-edit'>
                                                <a href="#">
                                                    <img src={delImgUrl} alt="" onClick={() => handleRemoveItem(item)} />
                                                </a>
                                            </td>
                                        </tr>
                                    ))
